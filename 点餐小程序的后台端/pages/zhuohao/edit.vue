  ...
  export default {
    ...
    methods: {
      ...
      async submitForm(value) {
        try {
          const res = await zhuohaoObj.saveTable({
            ...value,
            id: this.formDataId
          })
      ...
      getDetail(id) {
        ...
        db.collection(dbCollectionName).doc(id).field("createTime,tableNumber,orderQrCode,scene").get().then(async (res) => {
          const data = res.result.data[0]
          if (data) {
            this.formData = data
            await this.resolveQrCodeTempUrl(data.orderQrCode)
          }
        }).catch((err) => {
          uni.showModal({
            content: err.message || '请求服务失败',
