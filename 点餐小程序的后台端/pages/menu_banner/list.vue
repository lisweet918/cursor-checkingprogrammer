  ...
  <view>
    <view class="uni-header">
      <view class="uni-group">
        <view class="uni-title">点餐页广告图</view>
        <view class="uni-sub-title">管理小程序点餐页面顶部的轮播广告图</view>
      </view>
      <view class="uni-group">
    ...
    <view class="uni-container">
      <unicloud-db
        ...
        @load="onDbLoad"
      >
        <uni-table :loading="loading" :emptyText="error.message || '暂无点餐页轮播图'" border stripe>
          <uni-tr>
    ...
    methods: {
        ...
        uni.chooseImage({
          ...
          success: async (res) => {
            ...
            try {
              ...
              const fileName = `menu_banner/${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`
      ...
      async submitForm() {
        ...
        try {
          ...
          if (this.editId) {
            await db.collection('menu_banner').doc(this.editId).update(payload)
            uni.showToast({ title: '修改成功' })
          } else {
            payload.createTime = Date.now()
            await db.collection('menu_banner').add(payload)
      ...
      confirmDelete(id) {
        uni.showModal({
          ...
          success: async (res) => {
            if (res.confirm) {
              try {
                await db.collection('menu_banner').doc(id).remove()
